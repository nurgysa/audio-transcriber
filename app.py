...
class HistoryDialog(ctk.CTkToplevel):
    """Dialog for browsing transcription history — each entry is a folder on disk."""
    ...
    def __init__(self, parent):
        ...
        self._entry_list.grid(row=1, column=0, padx=16, pady=8, sticky="nsew")
        self._entry_list.grid_columnconfigure(0, weight=1)

        # --- Footer ---
        footer = ctk.CTkFrame(self, fg_color="transparent")
        footer.grid(row=2, column=0, padx=16, pady=(0, 12), sticky="ew")
        footer.grid_columnconfigure(1, weight=1)
    ...
    def _render_entries(self):
        ...
        self._lbl_count.configure(text=f"Записей: {len(entries)}")

        if not entries:
            ctk.CTkLabel(
                self._entry_list, text="Нет транскрипций",
                font=ctk.CTkFont(family=FONT, size=13),
                text_color=TEXT_SECONDARY,
        ...
        for i, entry in enumerate(entries):
            ...
            # Open folder button
            ctk.CTkButton(
                row, text="📂 Открыть", width=110, height=32, corner_radius=16,
                font=ctk.CTkFont(family=FONT, size=12),
