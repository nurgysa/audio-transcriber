...
- **Cloud (опционально):** транскрипция и диаризация через managed API (AssemblyAI). Запускается одним переключателем в Настройках, не требует GPU.
...
managed-API провайдеру. Сейчас поддерживается **AssemblyAI** (~$0.37/час
без диаризации, ~$0.65/час с диаризацией; включает русский и казахский,
лимит файла ~2 GB).

Шаги:

1. Создай аккаунт на https://www.assemblyai.com/ и получи API-ключ.
2. В приложении: **Настройки → Облако → ☑ Использовать облако**.
3. Вставь API-ключ в поле и закрой диалог.
...
Добавить других провайдеров (OpenAI Whisper, Deepgram, Replicate и т.п.)
можно через интерфейс [`providers/base.py`](providers/base.py) — одна
реализация + регистрация в [`providers/__init__.py`](providers/__init__.py).
