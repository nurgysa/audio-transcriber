...
| E2E / desktop tests | **WebdriverIO** via `tauri-driver` | WebdriverIO 9.x | Tauri 2's official desktop E2E story is the **WebDriver protocol**, not Playwright. `tauri-driver` (a small Rust binary, see `crates.io/crates/tauri-driver`) exposes a WebDriver endpoint that drives the actual native webview process; any WebDriver-protocol client connects to it. **WebdriverIO 9.x** is the recommended JS client — better TypeScript types and async API than Selenium WebDriver's classic sync model. Reference: the official Tauri 2 E2E docs (`v2.tauri.app/develop/tests/webdriver/`) ship WebdriverIO + Selenium examples; there is no `tauri test` CLI command and no first-party Playwright integration as of 2026-05. Playwright considered + rejected: it would only drive a Chromium instance loading the React app in isolation — useful for pure-webview regression but blind to native Tauri commands, IPC, file dialogs, system tray, and deep-link behaviour, which are the *whole point* of desktop E2E. Vitest already covers the React-component layer better (Vite-native, faster). |
